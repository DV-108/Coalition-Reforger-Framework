modded class Vehicle
{
	...
	void Vehicle(IEntitySource src, IEntity parent)
	{
		...
		CRF_VehicleGearscriptManager vehGSManager = CRF_VehicleGearscriptManager.GetInstance();
		if(vehGSManager)
		{
			GetGame().GetCallqueue().CallLater(SetVehicleGear, 500, false);
			GetGame().GetCallqueue().CallLater(CheckIfSpawnPassenger, 500, false);
	
			vehGSManager.AddVehicleToSpawnedArray(this);
		};
	}
	...
	void ~Vehicle()
	{
		...
		CRF_VehicleGearscriptManager vehGSManager = CRF_VehicleGearscriptManager.GetInstance();
		if (vehGSManager)
			vehGSManager.RemoveVehicleFromSpawnedArray(this);
